 ...
 */
var importNodes = (function (_importNodes) {
    var _importNodesWrapper = function importNodes() {
        return _importNodes.apply(this, arguments);
    };

    _importNodesWrapper.toString = function () {
        return _importNodes.toString();
    };

    return _importNodesWrapper;
})(function (orig, dest) {
    for (var i = 0; i < orig.childNodes.length; i++) {
        dest.appendChild(importNodes(orig.childNodes[i], dest.ownerDocument.importNode(orig.childNodes[i])));
    };
    return dest;
});
